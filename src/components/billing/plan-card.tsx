...
export function PlanCard({ currentPlan }: { currentPlan: PlanKey }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm font-medium text-muted-foreground">Instance Plan</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex items-center gap-2">
          <Badge variant="secondary" className="text-base px-3 py-1">{plan.name}</Badge>
          <span className="text-lg font-semibold">{formatCurrency(plan.price_eur)}/mo per instance</span>
        </div>
